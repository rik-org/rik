use crate::{cli::function_config::FnConfiguration, structs::WorkloadDefinition};
use curl::easy::Easy;
use firepilot::{
...
pub trait Runtime {
    fn run(&mut self, network_definition: &NetworkDefinition);
}
...
pub trait RuntimeManager {
    ...
    fn create_runtime(&self, workload_definition: WorkloadDefinition) -> Box<dyn Runtime>;

    fn create(&self, workload_definition: WorkloadDefinition, ip_allocator: IpAllocator) {
        let network = self.create_network(workload_definition.clone(), ip_allocator.clone());
        let mut runtime = self.create_runtime(workload_definition.clone());
