...
pub fn get(
    ...
    if let Ok(mut workloads) = RikRepository::find_all(connection, "/workload") {
        ...
        Ok(
            tiny_http::Response
                ::from_string("Cannot find workloads")
...
pub fn create(
    ...
    let name = format!("/workload/{}/{}/{}", workload.kind, namespace, workload.name);
    ...
    if
        let Ok(inserted_id) = RikRepository::insert(
            connection,
            &name,
        ...
        )
    {
        ...
        Ok(
            tiny_http::Response
                ::from_string(serde_json::to_string(&workload_id).unwrap())
                .with_header(tiny_http::Header::from_str("Content-Type: application/json").unwrap())
                .with_status_code(tiny_http::StatusCode::from(200))
        )
    } else {
        ...
        Ok(
            tiny_http::Response
                ::from_string("Cannot create workload")
                .with_status_code(tiny_http::StatusCode::from(500))
        )
    }
}
...
pub fn delete(
    ...
    if let Ok(workload) = RikRepository::find_one(connection, &delete_id, "/workload") {
        ...
        Ok(
            tiny_http::Response
                ::from_string(format!("Workload id {} not found", delete_id))
                .with_status_code(tiny_http::StatusCode::from(404))
        )
    }
}